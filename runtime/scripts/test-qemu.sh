...
#   build-dir   output of runtime/scripts/build-cross.sh (contains node)
...
[ -x "$NODE" ] || { echo "test-qemu: $NODE missing — run build-cross.sh first" >&2; exit 1; }

echo "== smoke"
"$QEMU" "$NODE" -e 'console.log("node", process.version, process.arch, process.platform)'
